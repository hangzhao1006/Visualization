// generate.mjs — Fetches Notion data and generates internship_sankey_flow.html
...
function buildSankeyData(entries) {
  ...
  while (i < sortedDates.length) {
    ...
    while (j < sortedDates.length) {
      ...
      if ((d - start) / 86400000 <= 1) { endStr = sortedDates[j]; j++; }
      else break;
    }
    const sDate = new Date(sortedDates[i] + "T00:00:00");
    const eDate = new Date(endStr + "T00:00:00");
    const label = sortedDates[i] === endStr
      ? `${sDate.getMonth() + 1}/${sDate.getDate()}`
      : `${sDate.getMonth() + 1}/${sDate.getDate()}–${eDate.getDate()}`;
  ...
  // Group by (company, status, stepIdx)
  const groups = {};
  entries.forEach((e) => {
    ...
    if (!groups[key]) groups[key] = { company: e.company, status, stepIdx, count: 0 };
    groups[key].count++;
  });

  // Build rows: [label, weight, s0, s1, ..., sN]
